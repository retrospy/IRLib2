...
#define IR_SEND_PROTOCOL_13		case 13: if(khz==38)khz=36; IRsendRC5_CDI::send(data,data2,khz); break;
#define IR_DECODE_PROTOCOL_13	if(IRdecodeRC5_CDI::decode()) return true;
#ifdef IRLIB_HAVE_COMBO
	#define PV_IR_DECODE_PROTOCOL_13 ,public virtual IRdecodeRC5_CDI
	#define PV_IR_SEND_PROTOCOL_13   ,public virtual IRsendRC5_CDI
#else
	#define PV_IR_DECODE_PROTOCOL_13  public virtual IRdecodeRC5_CDI
	#define PV_IR_SEND_PROTOCOL_13    public virtual IRsendRC5_CDI
#endif
#define RC5_CDI_T1		450

#ifdef IRLIBSENDBASE_H

class IRsendRC5_CDI: public virtual IRsendBase {
  public:
    void send(uint32_t data, uint8_t nBits=13, uint8_t kHz=36) {
      ...
      data = data << (32 - nBits);
      extent=0;
      mark(RC5_CDI_T1); // First start bit
...
//lines to maintain backward compatibility.
      //space(RC5_CDI_T1); // Second start bit
      //mark(RC5_CDI_T1); // Second start bit
      for (uint8_t i = 0; i < nBits; i++) {
        if (data & TOPBIT) {
          space(RC5_CDI_T1); mark(RC5_CDI_T1);// 1 is space, then mark
        } else {
          mark(RC5_CDI_T1);  space(RC5_CDI_T1);// 0 is mark, then space
        }
        data <<= 1;
      }
...
class IRdecodeRC5_CDI: public virtual IRdecodeRC {
  public:
    virtual bool decode(void) {
      IRLIB_ATTEMPT_MESSAGE(F("RC5-CDi"));
      ...
      used = 0;
      // Get start bits
      if (getRClevel(&used, RC5_CDI_T1) != MARK) return HEADER_MARK_ERROR(RC5_CDI_T1);
...
// to maintain backward compatibility.
      //if (getRClevel(&used, RC5_CDI_T1) != SPACE) return HEADER_SPACE_ERROR(RC5_CDI_T1);
      //if (getRClevel(&used, RC5_CDI_T1) != MARK) return HEADER_MARK_ERROR(RC5_CDI_T1);
      for (nBits = 0; offset < recvGlobal.decodeLength; nBits++) {
        RCLevel levelA = getRClevel(&used, RC5_CDI_T1);
        RCLevel levelB = getRClevel(&used, RC5_CDI_T1);
        if (levelA == SPACE && levelB == MARK) {
          data = (data << 1) | 1;   // 1 bit
        } else if (levelA == MARK && levelB == SPACE) {
          data <<= 1;               // zero bit
        } else return DATA_MARK_ERROR(RC5_CDI_T1);
      }
      // Success
      bits = nBits;
      value = data;
